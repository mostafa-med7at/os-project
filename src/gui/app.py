...
from src.scheduler.algorithms import round_robin, priority_scheduling
...
class SimpleSchedulerApp:
    ...
    # =========================================================
        ...
        self.notebook.add(self.tab_pr, text="Priority Results")
    ...
    def show_conclusion(self, rr_res, rr_avg, pr_res, pr_avg):
        rr_wt, rr_tat, rr_rt = rr_avg
        pr_wt, pr_tat, pr_rt = pr_avg

        rr_wts  = [r.wt for r in rr_res]
        pr_wts  = [r.wt for r in pr_res]
        rr_mean = sum(rr_wts) / len(rr_wts)
        pr_mean = sum(pr_wts) / len(pr_wts)
        ...
        urgent   = hp_pr < hp_rr * 0.9

        max_pri   = max(r.priority for r in pr_res)
        low_procs = [r for r in pr_res if r.priority == max_pri]
        starved   = any(r.wt > pr_wt * 2.0 for r in low_procs)
        low_names = ", ".join(r.pid for r in low_procs)

        better_wt = "Round Robin" if rr_wt <= pr_wt else "Priority Scheduling"
        better_rt = "Round Robin" if rr_rt <= pr_rt else "Priority Scheduling"
        recommend = better_wt

        analysis = "=== REQUIRED ANALYSIS QUESTIONS ===\n\n"
        analysis += f"1. Which algorithm gave better average waiting time?\n   -> {better_wt} (RR: {rr_wt:.2f}, PR: {pr_wt:.2f})\n\n"
        analysis += f"2. Which algorithm gave better response time?\n   -> {better_rt} (RR: {rr_rt:.2f}, PR: {pr_rt:.2f})\n\n"
        analysis += f"3. Did higher-priority processes gain significant advantage?\n   -> {'Yes' if urgent else 'No'}. High-priority avg WT -> Priority: {hp_pr:.2f} vs RR: {hp_rr:.2f}\n\n"
        analysis += f"4. Did Round Robin appear more balanced across all processes?\n   -> {'Yes' if rr_std < pr_std else 'No'}. WT Std Dev -> RR: {rr_std:.2f} vs PR: {pr_std:.2f}\n\n"
        ...
        analysis += f"6. Which algorithm would you recommend for the tested workload, and why?\n   -> {recommend}, because it provided the most optimal overall waiting time while {'balancing fairness' if rr_std < pr_std else 'prioritizing critical tasks'}.\n\n"
        ...
        analysis += f"- Round Robin {'DID' if rr_std < pr_std else 'DID NOT'} improve fairness.\n"
    ...
    def run(self):
        ...
        self.gantt_text.insert(tk.END, "=== Priority Scheduling Gantt Chart ===\n\n")
        ...
        self.show_conclusion(rr_res, rr_avg, pr_res, pr_avg)
